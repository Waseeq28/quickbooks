...
export async function getServerAuthzContext(): Promise<AuthzContext> {
  ...
  if (profileError) {
    throw Object.assign(new Error(`Failed to load profile: ${profileError.message}`), { status: 500 })
  }
  ...
  if (tmError) {
    throw Object.assign(new Error('Not a member of the current team'), { status: 403 })
  }
