...
export function AuthzProvider({ children, initialTeamId = null, initialRole = null }: { children: React.ReactNode, initialTeamId?: string | null, initialRole?: TeamRole | null }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!user) { setTeamId(null); setRole(null); return }
      ...
      if (!currentTeamId) { setRole(null); return }
      const { data: tm } = await supabase.from('team_members').select('role').eq('team_id', currentTeamId).eq('user_id', user.id).single()
...
export function PermissionGate({ action, children }: { action: Action, children: React.ReactNode }) {
  const { can } = useAuthz()
