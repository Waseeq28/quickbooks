...
export function TeamSettingsDialog({ open, onOpenChange, user, onTeamUpdate }: TeamSettingsDialogProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [currentTeam, setCurrentTeam] = useState(getCurrentTeam());
  const [currentTeamName, setCurrentTeamName] = useState(currentTeam?.name || "No Team Selected");
  const [currentTeamRole, setCurrentTeamRole] = useState(currentTeam ? mockUserTeams.find(ut => ut.teamId === currentTeam.id)?.role || "Member" : "Member");
  const [teamMembers, setTeamMembers] = useState(currentTeam ? getTeamMembers(currentTeam.id) : []);
  ...
  useEffect(() => {
    const handleTeamSwitch = () => {
      ...
      setCurrentTeamRole(newCurrentTeam ? mockUserTeams.find(ut => ut.teamId === newCurrentTeam.id)?.role || "Member" : "Member");
      ...
      setUserRole(newCurrentTeam ? mockUserTeams.find(ut => ut.teamId === newCurrentTeam.id)?.role || "Member" : "Member");
    };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg border-2 border-gray-800">
           ...
           <DialogTitle className="flex items-center gap-2">
             ...
             {currentTeamName === "No Team Selected" ? "Team Settings" : currentTeamName}
           </DialogTitle>
           <DialogDescription>
          ...
          <div className="space-y-4">
               ...
               <div className="flex gap-2">
                 <Input
                   ...
                   disabled={!isEditing}
                   placeholder={currentTeamName === "No Team Selected" ? "Enter team name" : currentTeamName}
                   className="flex-1"
                 />
                 {!isEditing && (
                   <Button
               ...
               {currentTeamName !== "No Team Selected" && (
                 <p className="text-xs text-muted-foreground">
                   Currently managing: {currentTeamName}
                 </p>
               )}
             </div>
