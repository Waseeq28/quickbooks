...
export async function DELETE() {
  try {
    ...
    if (role !== 'admin') {
      return NextResponse.json({ error: 'Only team admins can disconnect QuickBooks' }, { status: 403 })
    }
    await DatabaseService.deleteQuickBooksConnectionForTeam(teamId)
    return NextResponse.json({ success: true })
  } catch (e: any) {
    return NextResponse.json({ success: false, error: e?.message || 'Failed to disconnect' }, { status: 500 })
  }
}
