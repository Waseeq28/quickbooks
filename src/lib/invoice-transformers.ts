...
export function toSimpleInvoice(qbInvoice: QBInvoice): SimpleInvoice {
  ...
  const items = qbInvoice.Line.filter(
    (line) => line.DetailType === "SalesItemLineDetail",
  ).map((line) => ({
    description:
      line.Description || line.SalesItemLineDetail?.ItemRef?.name || "Item",
  ...
  return {
    ...
    customerName:
      qbInvoice.CustomerRef.name || `Customer ${qbInvoice.CustomerRef.value}`,
